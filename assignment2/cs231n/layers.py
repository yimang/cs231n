...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        var = np.sum((x - mean)**2, axis=0, keepdims=True)/N
        out = (out - mean)/np.sqrt(var + eps)
        cache = (gamma, x, mean, var, eps, out)
        out = out * gamma + beta
        ...
        for hh in range(H_prime):                           
            for ww in range(W_prime):                      
                for f in range(F):       
                    ...
                    vert_start = hh * stride
                    vert_end = hh * stride + HH
                    horiz_start = ww * stride
                    horiz_end = ww * stride + WW
        ...
        for hh in range(H_prime):                           
            for ww in range(W_prime):                      
                for f in range(F):                                        
                    ...
                    # Update gradients 
                    dx_pad[:,vert_start:vert_end, horiz_start:horiz_end] += w[f,:,:,:] * dout[i, f, hh, ww]
                    dw[f,:,:,:] += x_slice * dout[i, f, hh, ww]
                    db[f] += dout[i, f, hh, ww]                   
        ...
        for hh in range(H_prime):                           
            for ww in range(W_prime):                      
                for f in range(C):          
                    # Find the current slice
                    vert_start = hh * stride
                    vert_end = hh * stride + pool_height
                    horiz_start = ww * stride
                    horiz_end = ww * stride + pool_width
                    x_slice = x[i][f,vert_start:vert_end,horiz_start:horiz_end]
                    
                    out[i, f, hh, ww] = x_slice.max()
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
        ...
        for hh in range(H_prime):                           
            for ww in range(W_prime):                      
                for f in range(C):      
                    ...
                    horiz_start = ww * stride
                    horiz_end = ww * stride + pool_width
                    x_slice = x[f,vert_start:vert_end,horiz_start:horiz_end]
                    
                    # Update gradient
                    mask = (x_slice == np.max(x_slice))
                    dx[i, f, vert_start: vert_end, horiz_start: horiz_end] += mask * dout[i, f, hh, ww]   
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
