...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, hd in enumerate(hidden_dims):
            ...
            self.params['b'+str(i+1)] = weight_scale * np.zeros(hd)
    ...
    def loss(self, X, y=None):
        ...
        for lay in range(self.num_layers-1):
            if self.use_batchnorm:
                layer_input, ar_cache[lay] = affine_bn_relu_forward(layer_input, self.params['W'+str(lay+1)],                  
                                                                    self.params['b'+str(lay+1)],                                
                                                                    self.params['gamma'+str(lay+1)],                      
                                                                    self.params['beta'+str(lay+1)], self.bn_params[lay])
            else:
                layer_input, ar_cache[lay] = affine_relu_forward(layer_input, self.params['W'+str(lay+1)], 
                                                                 self.params['b'+str(lay+1)])

            if self.use_dropout:
                layer_input,  dp_cache[lay] = dropout_forward(layer_input, self.dropout_param)

        ar_out, ar_cache[self.num_layers] = affine_forward(layer_input, self.params['W'+str(self.num_layers)], 
        ...
        loss, dscores = softmax_loss(scores, y)
        dhout = dscores
        loss = loss + 0.5 * self.reg * np.sum(self.params['W'+str(self.num_layers)]**2)
        dx , dw , db = affine_backward(dhout , ar_cache[self.num_layers])
        grads['W'+str(self.num_layers)] = dw + self.reg * self.params['W'+str(self.num_layers)]
        grads['b'+str(self.num_layers)] = db
        dhout = dx
        for idx in range(self.num_layers-1):
            lay = self.num_layers - idx - 2
            loss = loss + 0.5 * self.reg * np.sum(self.params['W'+str(lay+1)]**2)
            if self.use_dropout:
            ...
            if self.use_batchnorm:
                grads['gamma'+str(lay+1)] = dgamma
                grads['beta'+str(lay+1)] = dbeta
            dhout = dx
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
